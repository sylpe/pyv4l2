...
def media_config_generator(media_config: MediaConfig) -> List[MediaConfig]:
    ...
    # Retrieve supported mbus_code / resolutions of the associated subdevice
    subdev = v4l2.SubDevice(source_entity.interface.dev_path)
    viddev = v4l2.VideoDevice(dest_entity.interface.dev_path)
    pad = 0  # Assuming pad index 0
    mbus_codes = subdev.get_formats(pad)

    # Generate a dict of MediaConfig for mbus_code / resolution
    media_configs = []
    for mbus_code in mbus_codes:
        pix_fmts = viddev.get_formats(v4l2.BufType.VIDEO_CAPTURE, mbus_code)
        frame_sizes = subdev.get_framesizes(pad, mbus_code)
